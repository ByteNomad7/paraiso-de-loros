...
- **Homepages unificadas (ES/PT/FR)** — las 3 reordenadas a un orden de secciones canónico único: Hero → Confianza (Trust/Why-choose) → Aves/Especies → Tarjetas comerciales (`sc-section`) → Proceso de adopción → Instalaciones/Envío → Blog preview → FAQ → Contacto CTA. PT: añadida sección Blog preview. FR (era escasa): construidas 3 secciones nuevas con copy francés — Proceso, FAQ y Contacto CTA — y reordenada. `lang-switcher.js`: eliminada fila duplicada `/nosotros → /fr/nos-installations` (queda solo `→ /fr/a-propos`).
- **Decisión de diseño:** "Adopción/Adoção/Adoption" se mantiene como **enlace directo de nav** (no dropdown) en los 3 idiomas, para consistencia con la nav ES desplegada en 253 páginas. Convertir solo PT/FR rompería la coherencia entre idiomas.
